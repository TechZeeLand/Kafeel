...
<div class="wrap section" style="padding-top:8px;">
  <div class="prose">
    ...
    <p>We ship nationwide with cash-on-delivery on every order, and deliver in <?= (int)DELIVERY_DAYS_MIN ?>–<?= (int)DELIVERY_DAYS_MAX ?> days. Delivery is <?= money(SHIPPING_INSIDE_DHAKA_FEE) ?> inside Dhaka and <?= money(SHIPPING_OUTSIDE_DHAKA_FEE) ?> outside Dhaka, with <?= money(SHIPPING_EXTRA_PER_KG) ?> added per additional kg once a parcel passes <?= (int)SHIPPING_FREE_WEIGHT_KG ?>kg. If something isn't right, unused items can be returned within 7 days of delivery — see our <a href="/refund-policy">refund & return policy</a> for details.</p>
    <h2>Questions?</h2>
